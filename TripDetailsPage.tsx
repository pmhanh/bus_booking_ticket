...
import { lockSeats, refreshSeatLock, releaseSeatLock } from '../api/seats';
...
export const TripDetailsPage = () => {
  ...
  useEffect(() => {
    if (!lockInfo || !accessToken || !id) return;
    return () => {
  ...
  const handleHoldSeats = async () => {
    ...
    try {
      const res = await lockSeats(
        Number(id),
        { seats: selectedSeats.map((s) => s.code), lockToken: lockInfo?.token },
        accessToken,
      );
  ...
  const handleReleaseHold = async () => {
    ...
    setLocking(true);
    try {
      await releaseSeatLock(Number(id), lockInfo.token, accessToken);
      setLockInfo(null);
    } catch (err) {
            ...
            <Card title="Thông tin liên hệ & tóm tắt">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  <Button onClick={handleHoldSeats} disabled={locking || !selectedSeats.length}>
                    {lockInfo ? "C?p nh?t gi? gh?" : "Gi? gh? 5 ph�t"}
                  </Button>
                  {lockInfo ? (
                    <>
                      <Button variant="secondary" onClick={handleRefreshHold} disabled={locking}>
                        Gia h?n +5p
                      </Button>
                      <Button variant="ghost" onClick={handleReleaseHold} disabled={locking}>
                        H?y gi?
                      </Button>
                      <span className="text-xs text-amber-200">
                        C�n {Math.floor(remainingSeconds / 60)}:{(remainingSeconds % 60).toString().padStart(2, "0")} ph�t
                      </span>
                    </>
                  ) : null}
                </div>
